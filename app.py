...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO tickets (tribune_name, supporter_name, gender, stadion, ticket_price, match_name, date_info) \
                    ...
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE tickets \
                                          SET tribune_name=:1, supporter_name=:2, gender=:3, stadion=:4, \
                                          ticket_price=:5, match_name=:6, date_info=:7 \
                                          WHERE id=:8;')
                            session.execute(query, {'1':tribune_name_baru, '2':supporter_name_baru, '3':gender_baru, '4':str(stadion_baru), 
